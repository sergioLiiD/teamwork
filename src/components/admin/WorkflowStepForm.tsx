...
import QuizSection from './workflow/QuizSection';
import TodoSection from './workflow/TodoSection';
...
const WorkflowStepForm: React.FC<WorkflowStepFormProps> = ({ onSubmit, onCancel }) => {
  ...
  const addAssignee = () => {
    if (newAssignee && !step.assignees.includes(newAssignee)) {
      setStep(prev => ({
        ...prev,
  ...
  const addNotification = () => {
    if (newNotification && !step.notifications.includes(newNotification)) {
      setStep(prev => ({
        ...prev,
      ...
      <div className="flex min-h-screen items-center justify-center p-4">
        <div className="w-full max-w-4xl rounded-lg bg-white shadow-xl">
          <div className="flex items-center justify-between border-b p-4">
            <h3 className="text-lg font-semibold">Create Workflow Step</h3>
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
              ...
              <div>
                ...
                <div className="mt-1 flex items-center space-x-2">
                  ...
                  <input
                    ...
                    onChange={e => setStep({ ...step, dueInDays: parseInt(e.target.value) })}
                    className="flex-1 rounded-md border border-gray-300 px-3 py-2 focus:border-blue-500 focus:outline-none"
                  />
                </div>
              </div>
            ...
            <div className="space-y-4">
              {activeTab === 'content' && (
                <>
                  ...
                  <div>
                    ...
                    <FileUpload
                      onFileSelect={files => setStep({
                        ...step,
                        content: { ...step.content, documents: files }
                      })}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={e => setStep({
                        ...step,
                        content: { ...step.content, videoUrl: e.target.value }
                      })}
                      className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 focus:border-blue-500 focus:outline-none"
                      placeholder="https://..."
                    />
              ...
              {activeTab === 'quiz' && (
                <QuizSection
                  questions={step.content.quiz?.questions || []}
                  onChange={questions => setStep({
                    ...step,
                    content: {
                      ...step.content,
                      quiz: { questions }
              ...
              {activeTab === 'todos' && (
                <TodoSection
                  ...
                  onChange={todos => setStep({
                    ...step,
                    content: { ...step.content, todos }
                  })}
                />
              )}
            </div>
            ...
            <div>
              ...
              {step.notifications.length > 0 && (
                <ul className="mt-2 space-y-2">
                  {step.notifications.map((email, index) => (
                    <li
                      key={index}
                      className="flex items-center justify-between rounded-md bg-gray-50 px-3 py-2"
                    >
